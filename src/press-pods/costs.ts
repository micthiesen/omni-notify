...
const LOGGER = Logger.named("PressPods.CostCounter");
...
export default class CostCounter {
  ...
  public recordLlmUsage(model: string, fn: string, usage: CompletionUsage | undefined) {
    return Effect.gen({ self: this }, function* () {
      if (!usage) {
        yield* LOGGER.warn("No usage data for LLM", { model });
        return;
      }
      const bareModel = model.split(":").pop() ?? model;
      if (!hasPrice(model)) {
        yield* LOGGER.debug(`No pricing for model ${bareModel}; counting as $0`);
      }
  ...
  public recordTtsUsage(model: string, fn: string, text: string) {
    return Effect.gen({ self: this }, function* () {
      ...
      yield* recordCostEventSafely({
        ...
        priceStatus:
          price === undefined ? "unknown" : price === 0 ? "free" : "estimated",
