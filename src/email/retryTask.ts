...
export interface EmailPipelineControls<R = never> {
  transport?: EmailTransport<unknown, R>;
  handlers?: Map<string, EmailHandler<unknown, R>>;
}
...
export default class EmailRetryTask<R = never> implements ScheduledTask<
  unknown,
  R | Docstore | Logger
> {
  ...
  private readonly getControls: () => EmailPipelineControls<R>;
  ...
  constructor(getControls: () => EmailPipelineControls<R>, logger: NamedLogger) {
  ...
  public readonly run = Effect.gen({ self: this }, function* () {
    ...
    if (due.length === 0) {
      yield* this.logger.debug("No email retries due");
    ...
    yield* this.logger.info(`Email retry pass: ${this.lastRunSummary}`);
  });
}
