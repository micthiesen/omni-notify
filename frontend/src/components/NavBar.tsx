...
function NavIcon({ name }: { name: IconName }) {
  const paths: Record<IconName, ReactNode> = {
    ...
    watch: <><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m10 9 5 3-5 3Z" /></>,
    listen: <><path d="M4 13a8 8 0 0 1 16 0" /><path d="M4 13v5a2 2 0 0 0 2 2h2v-8H4" /><path d="M20 13v5a2 2 0 0 1-2 2h-2v-8h4" /></>,
    research: <><path d="M4 5.5A2.5 2.5 0 0 1 6.5 3H11v16H6.5A2.5 2.5 0 0 0 4 21.5Z" /><path d="M20 5.5A2.5 2.5 0 0 0 17.5 3H13v16h4.5a2.5 2.5 0 0 1 2.5 2.5Z" /></>,
    more: <><circle cx="5" cy="12" r="1" fill="currentColor" /><circle cx="12" cy="12" r="1" fill="currentColor" /><circle cx="19" cy="12" r="1" fill="currentColor" /></>,
    operations: <><path d="M4 6h16M4 12h16M4 18h16" /><circle cx="8" cy="6" r="2" fill="var(--bg-card)" /><circle cx="16" cy="12" r="2" fill="var(--bg-card)" /><circle cx="10" cy="18" r="2" fill="var(--bg-card)" /></>,
    email: <><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m4 7 8 6 8-6" /></>,
    pets: <><circle cx="8" cy="8" r="2" /><circle cx="16" cy="8" r="2" /><circle cx="5" cy="13" r="2" /><circle cx="19" cy="13" r="2" /><path d="M8 18c0-3 2-5 4-5s4 2 4 5c0 2-2 3-4 3s-4-1-4-3Z" /></>,
    costs: <><circle cx="12" cy="12" r="9" /><path d="M15 8.5c-.8-.6-1.8-1-3-1-1.7 0-3 1-3 2.3 0 3.7 6 1.4 6 5.2 0 1.4-1.3 2.5-3.2 2.5-1.2 0-2.4-.4-3.3-1.2M12 5v14" /></>,
...
function ActionBadge({ value, title }: { value: number | null; title?: string }) {
  ...
  return (
    <span className={`nav-action-badge ${value === null ? "nav-action-unknown" : ""}`} title={title}>
      ...
      <aside className="app-sidebar">
        ...
        <nav className="sidebar-nav" aria-label="Primary navigation">
          <div className="sidebar-group">
            {PRIMARY_LINKS.map((item) => link(item))}
          </div>
          <div className="sidebar-group">
        ...
        <div className="sidebar-footer"><ConnectionBadge /></div>
      </aside>
      ...
      {moreOpen && (
        <div className="mobile-more-backdrop" onClick={() => setMoreOpen(false)}>
          ...
          >
            ...
            <div className="mobile-more-links">{MORE_LINKS.map((item) => link(item))}</div>
          </div>
        </div>
      )}
    </>
